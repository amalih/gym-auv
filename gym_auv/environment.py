...
from gym_auv.objects.rewarder import ColavRewarder, MultiRewarder
...
class BaseEnvironment(gym.Env, ABC):
    ...
    def __init__(self, env_config, test_mode=False, render_mode='2d', verbose=False):
        ...
        if not hasattr(self, '_rewarder_class'):
            self._rewarder_class = ColavRewarder, MultiRewarder
    ...
    def reset(self, save_history=True):
        ...
        if self.rewarder is None:
            ...
            self.rewarder = self._rewarder_class(self.test_mode)
    ...
    def step(self, action:list) -> (np.ndarray, float, bool, dict):
        ...
        obs = self.observe()
        #print('Requesting from environment, vessel: ', self.vessel.id)
        ...
        #print(vessel_data['speed_measurements'])

        # Receiving agent's reward
        reward = self.rewarder.calculate(self.vessel)
