...
class Vessel():
    ...
    def step(self, action) -> None:
        ...
        # 0: Forward, 1: Turning left, 2: Turning right
        if action == 0:
            self.input = np.array([1,0])
        if action == 1:
            self.input = np.array([1,0.05])
        if action == 2:
            self.input = np.array([1,-0.05])
    ...
    def perceive(self, obstacles:list) -> (np.ndarray, np.ndarray):
        ...
        if self._step_counter % self.config["sensor_interval_load_obstacles"] == 0:
            ...
            self._nearby_vessels_distances = list(filter(
                lambda obst: float(p0_point.distance(obst.boundary)), self.nearby_vessels
            ))


        self.reachable_vessels = list(filter(
            lambda obst: float(p0_point.distance(obst.boundary)) - self._width < sensor_range + 5 and (isinstance(obst,Vessel) or isinstance(obst,VesselObstacle)), obstacles
        ))
        ...
        if not self._nearby_obstacles:
            ...
            for obstacle in self._nearby_obstacles:
                if (isinstance(obstacle,Vessel) or isinstance(obstacle,VesselObstacle)):
                    #ship.reachable = True
                    if obstacle.speed != 0:
                         obstacle.risk = self.calculate_risk(obstacle)
                        # if isinstance(obstacle, Vessel):
                             #print(f'For ship {obstacle.index} of distance {self.calculate_distance(obstacle)},  risk = {obstacle.risk}')
                         #else:
                            # print(f'For rigid ship at distance {self.calculate_distance(obstacle)},  risk = {obstacle.risk}')
    ...
    def find_ship_domain(self,vessel):
        ...
        V_rel, DCPA, TCPA = self.calculate_TCPA_DCPA(vessel)
        #print(f'Vrel: {V_rel} -- dCPA: {DCPA} -- tCPA: {TCPA}')
        ...
        d1 = DCPA/2
        d2 = TCPA*V # + U
        d3 = max(DCPA, self._width*math.pow(V, 0.44)) # + U
        d4 = TCPA*V # + U
    ...
    def _isdone(self) -> bool:
        latest_data = self.req_latest_data()
        return any([
            latest_data['collision'],
            latest_data['reached_goal'],
            self._step_counter > self.config["max_timesteps"] and not self.test_mode,
        ])
    ...
    @property
    def nearby_distances(self):
        return [x for x in self._nearby_vessels_distances]
    ...
    def velocity(self) -> np.ndarray:
        ...
        return np.array(self._state[3:5])
